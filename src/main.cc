...
const uint8_t espreqProgram[] = {
    ...
    // 69420->B : Send(B) — triggers ESP32
    0x36,0x39,0x34,0x32,0x30,0x04,0x42,0x3F,
    0xE7,0x42,0x11,0x3F,
    // Stop
    0xD9,0x3F,
    // Lbl 2: SHOW — rows guarded by >= N*16 so sub() never gets partial chunk
    // Row 1: always shown (Output handles strings shorter than 16 fine)
    // Rows 2-8: only if length(Str1)>=row*16, guaranteeing full 16 chars for sub()
    0xD6,0x32,0x3F,
    0xE1,0x3F,
    // Row 1 always
    0xE0, 0x31, 0x2B, 0x31, 0x2B, 0xAA, 0x00, 0x11, 0x3F,
    // Row 2: if length(Str1)>=32
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x33, 0x32, 0x3F,
    0xE0, 0x32, 0x2B, 0x31, 0x2B, 0xBB, 0x0C, 0xAA, 0x00, 0x2B, 0x31, 0x37, 0x2B, 0x31, 0x36, 0x11, 0x11, 0x3F,
    // Row 3: if length(Str1)>=48
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x34, 0x38, 0x3F,
    0xE0, 0x33, 0x2B, 0x31, 0x2B, 0xBB, 0x0C, 0xAA, 0x00, 0x2B, 0x33, 0x33, 0x2B, 0x31, 0x36, 0x11, 0x11, 0x3F,
    // Row 4: if length(Str1)>=64
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x36, 0x34, 0x3F,
    0xE0, 0x34, 0x2B, 0x31, 0x2B, 0xBB, 0x0C, 0xAA, 0x00, 0x2B, 0x34, 0x39, 0x2B, 0x31, 0x36, 0x11, 0x11, 0x3F,
    // Row 5: if length(Str1)>=80
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x38, 0x30, 0x3F,
    0xE0, 0x35, 0x2B, 0x31, 0x2B, 0xBB, 0x0C, 0xAA, 0x00, 0x2B, 0x36, 0x35, 0x2B, 0x31, 0x36, 0x11, 0x11, 0x3F,
    // Row 6: if length(Str1)>=96
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x39, 0x36, 0x3F,
    0xE0, 0x36, 0x2B, 0x31, 0x2B, 0xBB, 0x0C, 0xAA, 0x00, 0x2B, 0x38, 0x31, 0x2B, 0x31, 0x36, 0x11, 0x11, 0x3F,
    // Row 7: if length(Str1)>=112
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x31, 0x31, 0x32, 0x3F,
    0xE0, 0x37, 0x2B, 0x31, 0x2B, 0xBB, 0x0C, 0xAA, 0x00, 0x2B, 0x39, 0x37, 0x2B, 0x31, 0x36, 0x11, 0x11, 0x3F,
    // Row 8: if length(Str1)>=128
    0xCE, 0xBB, 0x2B, 0xAA, 0x00, 0x11, 0x6E, 0x31, 0x32, 0x38, 0x3F,
...
void sendStr1ToCalc(const char *str) {
    ...
    for (uint16_t i = 0; i < strLen; i++) {
        ...
        if (c == '.')  { payload[2 + i] = 0x3A; continue; }  // TI decimal point
        if (c == '/')  { payload[2 + i] = 0x83; continue; }  // TI division token
        if (c == '+')  { payload[2 + i] = 0x70; continue; }  // TI addition token (0x2B=comma!)
        if (c == '(')  { payload[2 + i] = 0x10; continue; }  // TI left paren token
        if (c == ')')  { payload[2 + i] = 0x11; continue; }  // TI right paren token
        if (c == '=')  { payload[2 + i] = 0x6A; continue; }  // TI equals token
        payload[2 + i] = c;
    }
    sendPacket(0x15, payload, dataSize);
    delay(10);
...
void remoteSendStr0() {
    ...
    delay(600);             // wait for link menu to fully render
    sendKey(0x9B, true);    // kCapB — jump to String category
    delay(600);             // wait for string list to fully render
                ...
                case WAIT_PACKET:
                    if (cmd == 0xC9) {
                        ...
                        for (uint16_t i = 0; i < strLen; i++) {
                            ...
                            if (cc == '/')  { payload[2 + i] = 0x83; continue; }
                            if (cc == '+')  { payload[2 + i] = 0x70; continue; }
                            if (cc == '(')  { payload[2 + i] = 0x10; continue; }
                            if (cc == ')')  { payload[2 + i] = 0x11; continue; }
                            if (cc == '=')  { payload[2 + i] = 0x6A; continue; }
                            payload[2 + i] = cc;
                        }
